...
比如说认为 $S -> a S b$ 代表 `如果S满足文法，则aSb也满足文法`。
...
很奇怪，即使配置了全局代理，还是无法拉取镜像，docker desktop 也无法打开对应界面。
不清楚是不是像 raw.githubusercontent.com 那样 DNS 解析出现了问题需要修改 host 文件。
...
- 优点是在文章中嵌入数学公式要比 `Markdown` 方便很多，比如说对于同样的一个麦克斯韦方程组
...
- 而缺点也很明显，typst 毕竟是一门新兴的标记语言，
  所以对应的支持或是生态都不多。
  像是 VuePress、MkDocs、Hexo 乃至 Github 默认的 Jekyll 这类静态网站生成器都是以渲染 Markdown 为主。
  而目前主流的论文、简历模板则是以 Latex 居多，比如 [SJTUThsis](https://github.com/sjtug/SJTUThesis)。

  这就导致了 typst 在当前的应用比较受限。
  对我个人而言比较适合用来写当前的实验报告和数学作业，
  或是先用 typst 写好内容之后通过 pandoc 转为 markdown 或是其他的格式。
  （比如说这篇博文就是由 typst 写的，由 pandoc 转为 markdown）

===== git

之前对于 git 的了解仅限于 `git clone`,`git add`,`git commit`,`git push`,`git pull` 这几个。
但是最近逐渐频繁接触到了代码回滚这类的需要，就需要去学一些 git 的其他指令。

这周大概学了一下 `git checkout`切换分支、`git reset`回滚状态、`git merge`合并分支这几个。
还需要找到具体应用场景加以熟练。
...
因为随记页面需要，又重新接触了一下联邦宇宙，
这次没有用之前的 Mastodon，
而是凭着审美选择了 Misskey，
目前打算是拿来作为朋友圈和文件传输助手的平替，
不过目前来看不能完全替代朋友圈的社交属性。
不过拿来记点小点子挺不错的。
